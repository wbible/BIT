...
public class Sudoku extends JFrame implements MouseListener, ActionListener {
	...
	public void setNumber(String num) {
		...
		colorReset();
		for(NumLabel lb : list_all) setColor(num, lb);
	}
	...
	public void colorReset() {
		for(NumLabel lb : list_all) {
			if(!lb.isEditable()) { 
				...
				lb.setForeground(color_main);
			}
		}
	}
	
	public void setColor(String num, NumLabel lb_active) {
		for(NumLabel lb : getSet_sameGroup(lb_active)) {
			if(lb.getText().equals(num) && num != "" && lb.getId() != lb_active.getId()) {
				lb.setForeground(Color.RED);
				System.out.println(lb.getId() + " : " + lb_active.getId());
			}
		}	
	}
	...
	public void setLb_active(NumLabel lb) {
		lb_active = lb;
		Set<NumLabel> group = getSet_sameGroup(lb);
	}
